      ...
      <div className="modal-overlay">
        <div className="modal-card" style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-muted)', margin: '4px 0 16px 0' }}>
            Final Score: <span style={{ color: 'var(--color-p1)' }}>{p1Score}</span> - <span style={{ color: 'var(--color-p2)' }}>{p2Score}</span>
          </div>
          ...
          <div style={{ background: '#0b0f19', borderRadius: '12px', padding: '12px', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px' }}>
            <div style={{ fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)' }}>Match Recap</div>
            {history.map(g => (
              <div key={g.gameNumber} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <span>Game {g.gameNumber}: <strong>{g.stageName}</strong></span>
                <span style={{ fontWeight: 700, color: g.winner === 'P1' ? 'var(--color-p1)' : 'var(--color-p2)' }}>
                  {g.winner === 'P1' ? settings.p1Name : settings.p2Name} Won
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="modal-card">
        <div className="modal-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h3 className="modal-title">Record Game {currentGame} Winner</h3>
          </div>
        </div>

        <div style={{ textAlign: 'center', padding: '8px 0' }}>
          <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Playing On</div>
          ...
          <div style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: '10px' }}>Who won Game {currentGame}?</div>
